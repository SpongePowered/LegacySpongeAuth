  ...
  "Single Sign On" should {
    ...
    "via log in" in {
      ...
      "reject signatures that require verification" in new WithServer {
        ...
        // Log in
        val logIn = doLogIn(FakeRequest(POST, "/login"), FakeUser.username, FakeUser.password.get)
      ...
      "complete sso" in new WithServer {
        ...
        val logIn = doLogIn(request, FakeUser.username, FakeUser.password.get)
        assertAuthenticated(logIn)
    ...
    "via verify" in {
      ...
      "fail with invalid username" in {
        val verify = doLogIn(FakeRequest(POST, "/verify"), "urmom", FakeUser.password.get)
        status(verify) must equalTo(SEE_OTHER)
      }
      ...
      "reject invalid signatures" in {
        ...
        val showSignUp = route(this.app, request).get
        status(showSignUp) must equalTo(SEE_OTHER)
        getSSOToken(showSignUp).isEmpty must equalTo(true)
      }

      "complete sso" in new WithServer {
        val logInRequest = FakeRequest(POST, "/login")
        val logIn = doLogIn(logInRequest, FakeUser.username, FakeUser.password.get)
        assertAuthenticated(logIn)
        val authToken = getAuthToken(logIn).get.value

        val verifyRequest = FakeRequest(GET, "/verify" + SSOSpec.this.ssoQuery).withSession(authToken)
        ...
        val request = FakeRequest(POST, "/verify").withSSO(ssoToken.get).withSession(authToken)
        val verify = doLogIn(request, FakeUser.username, FakeUser.password.get)
        status(verify) must equalTo(SEE_OTHER)
